#!/usr/bin/env python3
"""Install, verify, and manage DeepSeek as a native Codex worker."""
...
CREDENTIAL_TARGET = "codex-deepseek-worker-api-key"
OFFICIAL_SETUP_URL = "https://cdn.deepseek.com/api-docs/codex-deepseek-setup-en.sh"
PROVIDER_BEGIN = "# BEGIN CODEX-DEEPSEEK-WORKER PROVIDER"
PROVIDER_END = "# END CODEX-DEEPSEEK-WORKER PROVIDER"
ROLE_BEGIN = "# BEGIN CODEX-DEEPSEEK-WORKER ROLE"
ROLE_END = "# END CODEX-DEEPSEEK-WORKER ROLE"
...
def codex_version_text(codex_bin: str) -> str:
    ...
    if proc.returncode != 0 or not text:
        raise ManagerError("codex_version_unknown", "Could not read the Codex desktop bundled runtime version.")
    return text
...
def _macos_store_credential(secret: str) -> None:
    ...
    if proc.returncode != 0:
        raise ManagerError("credential_write_failed", "Could not store the API key in macOS Keychain.")
...
def _windows_store_credential(secret: str) -> None:
    ...
    if not advapi32.CredWriteW(ctypes.byref(credential), 0):
        ...
        raise ManagerError(
            ...
            f"Could not store the API key in Windows Credential Manager (error {error}).",
        )
...
def read_credential_key() -> str | None:
    ...
    raise ManagerError("unsupported_platform", "Credential storage currently supports macOS and Windows only.")
...
def store_credential_key(secret: str) -> None:
    if not secret.startswith("sk-"):
        raise ManagerError("invalid_api_key", "A DeepSeek API key must start with sk-.")
    backend = credential_backend()
    if backend == "macos-keychain":
...
def parse_toml_text(text: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise ManagerError("invalid_config", f"Could not parse config.toml: {exc}") from exc
...
Do not claim to inspect images, videos, screenshots, or other visual inputs. If visual evidence is required and the parent did not provide a textual description, report that limitation.
Do not spawn additional subagents unless the user or parent explicitly requests nested delegation.
Do not make final product, security, legal, financial, deployment, or publishing decisions. Return evidence and tradeoffs to the parent Codex agent.
...
def fetch_official_deepseek_model() -> dict[str, Any]:
    request = urllib.request.Request(
        OFFICIAL_SETUP_URL,
        headers={"User-Agent": "BeatAPI-codex-deepseek-worker/1.0"},
    )
    try:
        with urllib.request.urlopen(request, timeout=30) as response:
            script = response.read().decode("utf-8")
    except Exception as exc:
        raise ManagerError("official_catalog_unavailable", "Could not fetch the official DeepSeek Codex model catalog.") from exc
    match = re.search(
    ...
    if not match:
        raise ManagerError("official_catalog_changed", "The official DeepSeek setup format changed. No remote script was executed.")
    try:
        payload = json.loads(match.group(1))
    except json.JSONDecodeError as exc:
        raise ManagerError("official_catalog_invalid", "The official DeepSeek model catalog is not valid JSON.") from exc
...
def run_codex_models(codex_bin: str, paths: Paths) -> dict[str, Any]:
    ...
    if proc.returncode != 0:
        raise ManagerError("codex_catalog_failed", "Codex could not read the current model catalog.", {"stderr": proc.stderr[-800:]})
    try:
        return json.loads(proc.stdout)
    except json.JSONDecodeError as exc:
        raise ManagerError("codex_catalog_invalid", "Codex returned an invalid JSON model catalog.") from exc
...
def merged_catalog(base: dict[str, Any], deepseek_model: dict[str, Any], parent_model: str) -> dict[str, Any]:
    ...
    for model in models:
        if model.get("slug") == parent_model:
            ...
            break
    if not parent_found:
        raise ManagerError("parent_model_missing", f"The catalog does not contain parent model {parent_model}.")
...
def install(paths: Paths, codex_bin: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not parent_model:
            raise ManagerError("parent_model_unconfigured", "Codex config does not define a non-DeepSeek parent model.")
...
def native_test(paths: Paths, codex_bin: str) -> dict[str, Any]:
    ...
    if proc.returncode != 0:
        raise ManagerError(
            "native_test_failed",
            "Native spawn_agent verification failed in a fresh Codex task.",
...
def setup(paths: Paths, codex_bin: str, api_key_stdin: bool, skip_live_test: bool) -> dict[str, Any]:
    if not credential_available():
        raise ManagerError("unsupported", "Credential storage currently supports macOS and Windows only.")
    ...
    if not credential_has_key():
        ...
        if not secret:
            raise ManagerError("credential_missing", "No API key was provided on standard input.")
...
def uninstall(paths: Paths, remove_credential: bool) -> dict[str, Any]:
    ...
    if paths.catalog.is_file() and sha256_bytes(paths.catalog.read_bytes()) != manifest.get("catalog_sha256"):
        raise ManagerError(
            "conflict",
            "The managed model catalog was modified; uninstall was refused.",
...
def try_acquire_file_lock(lock_file) -> bool:
    ...
    if msvcrt is not None:
        ...
        try:
            ...
            return False
    raise ManagerError("unsupported_platform", "No supported file-lock implementation is available on this platform.")
        ...
        while True:
            ...
            if remaining <= 0:
                raise ManagerError(
                    ...
                    "Another DeepSeek worker configuration operation is still running. Retry later.",
                )
...
def main() -> int:
    ...
    if sys.argv[1:] == ["_credential-get"]:
        try:
            ...
            if not secret:
                print("DeepSeek API key is missing.", file=sys.stderr)
                return 2
    ...
    try:
        ...
        emit(result("timeout", message="The operation timed out. No credential was printed."), args.json)
        return 3
    except Exception as exc:
